...
#define STM32U575ZITXQ_NUCLEO
#ifdef STM32U575ZITXQ_NUCLEO
  manuf_data[14] = 0x7B; /* BoardType */
#endif /* STM32U575ZITXQ_NUCLEO */
  ...
  if (OTA_RemainingSize != 0)
  {
    ...
    if (RetValue != 0)
    {
      ...
      if (RetValue == 1)
      {
        ...
        MOTENV1_PRINTF("%s will restart after the disconnection\r\n", MOTENV1_PACKAGENAME);
        HAL_Delay(1000);
        NeedToSwapBanks = 1;
      }
    }
...
void ext_config_banks_swap_command_callback(void)
{
  if (CurrentActiveBank == 1)
  {
    ...
    {
      MOTENV1_PRINTF("Swapping to Bank%d\n", (CurrentActiveBank == 1) ? 0 : 1);
      MOTENV1_PRINTF("%s will restart after the disconnection\r\n", MOTENV1_PACKAGENAME);
      NeedToSwapBanks = 1;
    }
    else
    {
      MOTENV1_PRINTF("Not Valid fw on Bank%d\n\tCommand Rejected\n", (CurrentActiveBank == 1) ? 0 : 1);
      MOTENV1_PRINTF("\tLoad a Firmware on Bank%d\n", (CurrentActiveBank == 1) ? 0 : 1);
    }
  }
