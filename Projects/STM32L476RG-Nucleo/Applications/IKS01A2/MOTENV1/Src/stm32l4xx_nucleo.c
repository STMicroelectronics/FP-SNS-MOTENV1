...
#endif
__weak HAL_StatusTypeDef USART2_UART_Init(UART_HandleTypeDef* huart);
/**
 * @}
 */ 
...
int32_t BSP_COM_Init(COM_TypeDef COM) 
{
  ...
  {  
    ...
    if (USART2_UART_Init(&hcom_uart[COM]))
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
  }
...
int32_t BSP_COM_DeInit(COM_TypeDef COM)
{
  ...
  {  
    ...
    #if (USE_HAL_UART_REGISTER_CALLBACKS == 0U)  
      HAL_UART_MspDeInit(&hcom_uart[COM]);  
...
//  huart->AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
  ...
  huart->Init.BaudRate = huart2.Init.BaudRate;
  huart->Init.WordLength = huart2.Init.WordLength;
  huart->Init.StopBits = huart2.Init.StopBits;
  huart->Init.Parity = huart2.Init.Parity;
  huart->Init.Mode = huart2.Init.Mode;
  huart->Init.HwFlowCtl = huart2.Init.HwFlowCtl;
  huart->Init.OverSampling = huart2.Init.OverSampling;
  huart->Init.OneBitSampling = huart2.Init.OneBitSampling;
  huart->AdvancedInit.AdvFeatureInit = huart2.AdvancedInit.AdvFeatureInit;
...
int32_t BSP_COM_RegisterDefaultMspCallbacks(COM_TypeDef COM)
{
  ...
  {  
    ...
    if(HAL_UART_RegisterCallback(&hcom_uart[COM], HAL_UART_MSPINIT_CB_ID, HAL_UART_MspInit) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else if(HAL_UART_RegisterCallback(&hcom_uart[COM], HAL_UART_MSPDEINIT_CB_ID, HAL_UART_MspDeInit) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else
