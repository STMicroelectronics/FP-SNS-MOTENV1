/**
  ...
  * @brief   Bootloader for STM32L476RG and STM32F401RE and script files for binary flashing.
  ******************************************************************************
  * @attention
  *
...
This folder contains the script files (.sh), to use for all the supported IDEs (EWARM, MDK-ARM 
and STM32CubeIDE), for installing the BootLoader and flashing the MOTENV1 firmware on a 
STM32L476RG or STM32F401RE MCU.
These scripts also create a *_BL.bin file for STM32L476RG or STM32F401RE MCU composed by the BootLoader and the 
MOTENV1 firmware.

The scripts *.sh that makes the following operations:
	- Full Flash Erase
...
| STM32L476RG |
 --------------
The firmware contained in the folder Utilities\BootLoader\STM32L476RG is the Boot Loader that must be flashed at the 
...
For STM32L476RG the 1Mbytes-Flash is divided in 
...
| STM32F401RE |
 --------------
 The firmware contained in the folder Utilities\BootLoader\STM32F4xxRE is the Boot Loader that must be flashed at the 
Flash start address (0x08000000) and has the purpose to apply and run the Firmware-Over-The-Air 
(FOTA) receveid. 
It runs the program normally if no update is received.
